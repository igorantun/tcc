...
%-------------------------------------------------------------------------
% Modelo adaptado especificamente para o contexto do trabalho de PSG do curso de 
% Sistemas de Informação da EACH-USP, a partir de adaptação feita para o PPgSI-EACH-USP % por Marcelo Fantinato, com auxílio dos Professores Norton T. Roman, Helton
...
%
% 26/06/2021 - adaptação para o o trabalho de PSG por Sarajane Marques Peres
%-------------------------------------------------------------------------
...
%
% Colocar o ano de entrega da monografia 
%
%
...
\newline \newline \newline 
%-------------------------------------------------------------------------
% gerencie as opções abaixo conforme seu caso
% para PSG 1 - relatório parcial
%Relatório parcial de atividades de trabalho de conclusão de curso anual apresentado como parte dos requisitos necessários para cumprimento da disciplina ACH 2017 – Projeto Supervisionado ou de Graduação I.
% para PSG 1 - monografia final
...
%\newline \newline Modalidade: TCC curto (1 semestre) - individual
%\newline \newline Modalidade: TCC curto (1 semestre) - em grupo
\newline \newline Modalidade: TCC longo (1 ano) - individual
%\newline \newline Modalidade: TCC longo (1 ano) - em grupo
%-------------------------------------------------------------------------
%-------------------------------------------------------------------------
\newline }
% ---
...
%
% Página a ser usada apenas quando a monografia for catalogada na biblioteca.
%
...
%
%-------------------------------------------------------------------------
%\begin{fichacatalografica} %-- retire esse comentário
%    \includepdf{fig_ficha_catalografica.pdf} %-- retire esse comentário
%\end{fichacatalografica} %-- retire esse comentário
% ---
% ---
% Inserir folha de aprovação
% ---
% necessário apenas para trabalhos que serão catalogados na biblioteca - tirar os comentários das linhas marcadas 
%\begin{folhadeaprovacao} %-- tirar esse comentário
%-------------------------------------------------------------------------
...
%
%-------------------------------------------------------------------------
%\noindent Monografia de autoria de Fulano de Tal, sob o título \textbf{``\imprimirtitulo''}, apresentada à Escola de Artes, Ciências e Humanidades da Universidade de São Paulo, para obtenção do título de Bacharel em Sistemas de Informação, aprovada em \rule{0.85cm}{0.5pt} de \rule{3.5cm}{0.5pt} de \rule{1.25cm}{0.5pt} pela comissão julgadora constituída pelos doutores: %-- tirar esse comentário
%\vspace*{3cm} %-- tirar esse comentário
%\begin{center} %-- tirar esse comentário
%\assinatura{Prof. Dr. \\ Instituição \\ Presidente} %-- tirar esse comentário
%\assinatura{Prof. Dr. \\ Instituição} %-- tirar esse comentário
%\end{center} %-- tirar esse comentário
%\end{folhadeaprovacao} %-- tirar esse comentário
...
% 
% Troque os seguintes campos pelos dados de sua monografia (mantendo a 
...
A hipótese do agendamento propõe a ideia de que grandes veículos midiáticos, apesar de não influenciarem diretamente \textit{o que} as pessoas pensam sobre determinadas questões, possuem o grande poder de determinar \textit{quais} serão as questões de destaque na sociedade em um determinado momento, em detrimento de outras. No entanto, com a adoção em massa das mídias sociais, o público é exposto a uma variedade maior de fontes de notícia e possui uma influência direta na importância que é dada a um tema. O presente projeto busca entender, através de um estudo longitudinal e da aplicação de técnicas automatizadas de raspagem de dados em massa, a correlação entre a importância dada pela agenda midiática em portais jornalísticos e a que é dada pela agenda pública, no contexto das eleições presidenciais de 2022 no Brasil.
...
%-------------------------------------------------------------------------
\chapter{Introdução}
A hipótese do agendamento (\textit{agenda-setting}) propõe a ideia de que grandes veículos midiáticos, apesar de não influenciarem diretamente \textit{o que} as pessoas pensam sobre determinadas questões, possuem o grande poder de determinar \textit{quais} serão as questões de destaque na sociedade em um determinado momento, em detrimento de outras \cite{mccombs}. Com a adoção em massa das mídias sociais, no entanto, o público geral é exposto a uma variedade crescente de fontes de notícias, sejam essas legítimas ou falsas \cite{sterrett}.  

A decisão editorial, agora decentralizada, é diretamente influenciada pelo engajamento dado por usuários nas redes. Essa configuração pode gerar, em determinadas questões e momentos, um descasamento entre o que é priorizado no debate público e o que é destacado pela mídia, como observado nas eleições de 2010 por \citeonline{nunomura}.

O presente trabalho busca entender, através de um estudo longitudinal e da aplicação de técnicas automatizadas de raspagem de dados, a correlação entre a importância dada pela agenda midiática em portais jornalísticos e a que é atribuída pela agenda pública nas redes sociais, no contexto das eleições presidenciais de 2022 no Brasil.


\section{Justificativa para a pesquisa}
Ao ditar quais serão as questões de maior relevância para o debate público, um agente efetivamente limita as opções de temas que serão tratados na sociedade e suas políticas públicas. Estudar o impacto do agendamento nas plataformas digitais sociais e midiáticas, portanto, é uma importante ferramenta para entendermos sua influência no contexto da mudança social.

Através da aplicação de técnicas de raspagem de dados e automação, é possível acompanhar em tempo real os tópicos noticiados nos jornais digitais e os que estão em pauta nas redes sociais. Uma vez agregados, pode-se analisar os dados obtidos por diferentes perspectivas, buscando padrões e correlações entre suas agendas.


\section{Proposição da pesquisa}
O presente estudo de caso possui a intenção de analisar os efeitos do agendamento midiático no contexto das eleições presidenciais no Brasil em 2022. Através da observação conjunta do destaque dado a uma publicação nos cinco principais portais jornalísticos do país (\textit{Estadão}, \textit{Folha de S. Paulo}, \textit{g1}, \textit{UOL} e \textit{Veja}) e o engajamento obtido por estas nas redes sociais \textit{Facebook} e \textit{Twitter}, pretende-se observar a eventual correlação entre as agendas.
...
\section{Método}
A fim de comparar a saliência dada a um determinado tema nos portais jornalísticos e nas redes sociais, é necessário primeiro obter os dados brutos necessários e tratá-los de forma a possibilitar a análise. Para cada meio, a obtenção dos dados se deu de forma distinta.

Para a observação dos portais jornalísticos, foi desenvolvido um \textit{script} automatizado, executado em intervalos regulares de 1 hora, que aplica técnicas de raspagem de dados objetivando o armazenamento histórico das matérias publicadas na página inicial de cada um dos cinco principais portais jornalísticos do país: \textit{Estadão}, \textit{Folha de S. Paulo}, \textit{g1}, \textit{UOL} e \textit{Veja}. A cada matéria identificada, são coletadas informações como seu endereço de acesso, posição hierárquica na tela e total de horas em que se manteve visível.

Na rede social Twitter foi utilizada sua \textit{API} pública \cite{twitterapi}, que permite a busca de publicações específicas e análise das interações obtidas por estas. A busca foi então utilizada para encontrar publicações (\textit{tweets}) que referenciem o \textit{link} das matérias identificadas pelo \textit{script} acima. Os metadados de engajamento, como "curtidas", respostas e republicações (\textit{retweets}) são então armazenados.

Os dados do Facebook, por sua vez, foram obtidos com o uso da ferramenta CrowdTangle, que oferece acesso ao arquivo histórico de publicações do Facebook e também a seus dados atualizados. Através da mesma, é possível buscar publicações que contenham um \textit{link} específico, e assim como com o Twitter, armazenar suas métricas de engajamento.
